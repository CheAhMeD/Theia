...
// Storage
Storage storage(SD_MMC_CLK, SD_MMC_CMD, SD_MMC_D0, SD_MMC_D1, SD_MMC_D2, SD_MMC_D3);
// Lepton Camera
...
static void lv_sys_timer_3s_cb(lv_timer_t *timer) {
  ...
  if ((get_var_act_scr_id() == SCREEN_ID_MAIN_SCREEN) && !get_var_lep_snap_to_sd()) {
    if (storage.sdCardInserted()) {
      // SD Card still inserted
      sdCardState = storage.sdCardCheckSpace();
    } else {
      // SD Card removed
      sdCardState = 0;
    }
...
void setup() {
  ...
  displayBootScreen();
  // USB CDC
  displayBootLogText("Starting USB CDC...");
  Serial.begin(921600);
  Serial.setTxBufferSize(1024);
  Serial.flush();
  delay(200);
  // LED
  displayBootLogText("Starting StatusLED...");
  statusled.init();
  statusled.setStatus(0);
  delay(200);
  displayBootLogText("Starting Storage...");
  // Init SD card & EEPROM
  sdCardState = storage.init();
  delay(200);
  displayBootLogText("Checking SD Card...");
  set_var_sd_card_state(sdCardState);
  if (sdCardState != 0) {
    // Check if lepton dir exists otherwise create it
    storage.sdCardCheckLeptonDir();
    // Check if .theia file exists otherwise create it
    storage.sdCardCheckTheiaFile();
  }
  delay(200);
  // get settings from persistant storage 
  displayBootLogText("Loading Settings...");
  storage.psLoadInitValues();
  delay(200);
  //LVGL
  lvgl_init();
...
void lepTask(void *pvParameters) {
  ...
  while (1) {
    ...
    if (get_var_act_scr_id() == SCREEN_ID_MAIN_SCREEN) {
      ...
      if (get_var_lep_snap_to_sd()) {
        ...
        char *img_name;
        char index_buffer[5];
        // get last file index from storage
        uint16_t index = storage.psGetImageIndex() + 1;
        ...
        if (success) {
          ...
          storage.psCommitImageIndex(index);
        }
...
void loop() {
  ...
  if (get_var_set_gain_trig()) {
    ...
    storage.psCommitByte(PS_RAM_LEP_GAIN_MODE_ADDR, gain);
  ...
  /* Update storage triggers handeling */
  if (get_var_update_palette_trig()) {
    set_var_update_palette_trig(false);
    uint8_t pal = get_var_palette_num();
    storage.psCommitByte(PS_RAM_PALETTE_NUM_ADDR, pal);
  }
  if (get_var_update_temp_unit_trig()) {
    set_var_update_temp_unit_trig(false);
    uint8_t unit = get_var_cur_temp_format();
    storage.psCommitByte(PS_RAM_LEP_TEMP_UNIT_ADDR, unit);
  }
  if (get_var_update_img_format_trig()) {
    set_var_update_img_format_trig(false);
    uint8_t format = (uint8_t) get_var_image_format();
    storage.psCommitByte(PS_RAM_LEP_IMG_FORMAT_ADDR, format);
  }
  /* Browse Screen handeling */
  // if browse start triggered
  if (get_var_browse_start_trig()) {
    set_var_browse_start_trig(false);
    storage.startBrowseSession();
  }
