...
uint8_t palette_num = 0;         // Current colormap index for fast lookup
...
bool update_palette_trig = false;
bool update_temp_unit_trig = false;
bool update_img_format_trig = false;
bool run_ffc_trig = false;
bool set_gain_trig = false;
bool notification_enable = false;
bool image_format = true;  //True: BMP, False: RAW
...
void load_palette(uint8_t n) {
  if (n > GUI_CMAP_PALETTE_COUNT) {
    ESP_LOGE(TAG, "Failed to load palette (loading Arctic)");
    n = 0;
  }
...
bool get_var_update_palette_trig(){
  return update_palette_trig;
}

void set_var_update_palette_trig(bool trig){
  update_palette_trig = trig;
}

bool get_var_update_temp_unit_trig(){
  return update_temp_unit_trig;
}

void set_var_update_temp_unit_trig(bool trig){
  update_temp_unit_trig = trig;
}

bool get_var_update_img_format_trig(){
  return update_img_format_trig;
}

void set_var_update_img_format_trig(bool trig){
  update_img_format_trig = trig;
}
